...
from AyiinXd import CMD_HELP, COUNT_PM, LASTMSG, LOGS, PM_AUTO_BAN, PM_LIMIT, bot, logo
from AyiinXd.events import ayiin_cmd
from AyiinXd.utils import edit_delete, edit_or_reply

DEF_UNAPPROVED_MSG = (
    f"╔═════════════════════╗\n"
    f"│ ㅤ 𖣘𝚂𝙴𝙻𝙰𝙼𝙰𝚃 𝙳𝙰𝚃𝙰𝙽𝙶 𖣘ㅤ  ㅤ   \n"
    f"╚═════════════════════╝\n"
    f"⍟ 𝙹𝙰𝙽𝙶𝙰𝙽 𝚂𝙿𝙰𝙼 𝙲𝙷𝙰𝚃 𝙼𝙰𝙹𝙸𝙺𝙰𝙽 𝙶𝚄𝙰 \n"
...
async def approvepm(apprvpm):
    ...
    try:
        ...
        return await edit_delete(apprvpm, "**pesan diterima cuy**")

    await edit_delete(
        apprvpm, f"**Pesan dari ** [{name0}](tg://user?id={uid}) **diterima. ", 5
    )
