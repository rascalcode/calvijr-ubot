...
from AyiinXd import call_py, owner
...
async def join_(event):
    ...
    if len(event.text.split()) > 1:
        ...
        except Exception as ayiin:
            await edit_delete(event, f"**ERROR:** `{ayiin}`", 30)
    else:
        chat = event.chat_id
    ...
    try:
        await xnxx.edit(f"**[Success] - Joined Voice Chat {chat}**")
...
async def leavevc(event):
    ...
    if from_user:
        try:
            ...
            pass
        await xnxx.edit(f"**[Success] - Leave Voice Chat {chat_id}**")
    else:
